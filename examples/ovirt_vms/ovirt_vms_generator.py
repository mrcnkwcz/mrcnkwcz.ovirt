...
def generate_vms_from_profiles(profiles_input_file, profiles_output_file):
    """Generate func"""
    with open(profiles_input_file, "r") as f:
    ...
    for profile_name, profile_data in profiles.items():
        ...
        for i in range(1, count + 1):
            ...
            vm_list.append(vm)
    with open(profiles_output_file, "w") as f:
        yaml.dump({VMS_VAR: vm_list}, f, Dumper=IndentDumper, default_flow_style=False, sort_keys=False, width=120, explicit_start=True)

    print(f"VMs list saved to: {profiles_output_file}")

def main():
    """Main func"""
    parser = argparse.ArgumentParser(description="CLI for generating of list VM configuration from profiles")
    parser.add_argument(
        "-f", "--profiles", required=True, type=str,
        help="YAML file with profiles"
    )
    parser.add_argument(
        "-o", "--output", default="ovirt_vms.yml", type=str,
